/* Curated sample data for the prototype */
window.APP_DATA = (() => {
  const rows = [
  ...
  const upNext = rows.map((r, i) => ({ id: i + 1, ...r }));

  // Previously on device — used for "will overwrite" fate on Today
  const onDevice = [
  ...
  return { upNext, onDevice, deviceCapacityMB: 6200 };
})();
